...
#ifndef DESUGAR_LIBDESUGAR_HH
# define DESUGAR_LIBDESUGAR_HH
...
namespace desugar
{
  ...
  /** \brief Add runtime checks of array bounds.
      ...
      \return the AST with bound checks, with bindings and type-checked. */

  template <typename A>
  A*
  bound_checks_add(const A& tree);

  /** \brief Remove the syntactic sugar from an AST without
      recomputing its bindings nor its types.

      This function acts like desugar::bound_checks_add, but stops
      just after the tranformation step (in fact,
      desugar::bound_checks_add is built upon this function).  It is
      meant to be used as a test of BoundCheckingVisitor (i.e., even
      if the tree with bound checks is badly bound or typed, it can
      still be pretty-printed).

      \param tree AST on which bound checks are to be added

      \return the AST with bound checks.  */
  template <typename A>
  A*
  raw_bound_checks_add(const A& tree);

} // namespace desugar

#endif // !DESUGAR_LIBDESUGAR_HH
