...
namespace desugar
{
  ...
  {
    // Add array bound checking code.
    BoundCheckingVisitor add_bound_checks;
    add_bound_checks(tree);
    return dynamic_cast<A*>(add_bound_checks.result_get());
  }
  ...
  {
    // Add bound checks.
    A* transformed = raw_bound_checks_add(tree);
  ...
  /// Explicit instantiations.
  template ast::DecsList* raw_bound_checks_add(const ast::DecsList& t);
  template ast::DecsList* bound_checks_add(const ast::DecsList& t);
