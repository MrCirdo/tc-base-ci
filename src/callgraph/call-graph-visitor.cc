...
namespace callgraph
{
  ...
  {
    return const_cast<CallGraph*>
           (create(const_cast<const ast::Ast&>(tree)));
  }
  ...
  {
    ast::FunctionDec* called = const_cast<ast::FunctionDec*>(e.def_get());
    assertion(called);
    callgraph->fundec_link(caller, called);
