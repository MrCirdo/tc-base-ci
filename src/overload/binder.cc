...
namespace overload
{
  ...
  {
    std::list <ast::FunctionDec*> potential_defs =
      overfuns_.get(e.name_get());
    if (potential_defs.empty())
      return undeclared("function", e);
    (*overfun_bindings_)[&e] = potential_defs;
    // Continue the walk.
    ast::DefaultVisitor::operator()(e);
  }
