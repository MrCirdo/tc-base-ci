...
namespace tree
{
  ...
  void Tree::replace_by(const rTree& tree)
  {
    parent_get_assert();
    precondition(tree);
    ...
    tree_list_type::iterator me =
      ranges::find(siblings, this, &misc::ref<tree::Tree>::get);
  ...
  std::ostream& Tree::dump(std::ostream& o) const
  {
    tag_print(o);
    if (!children_.empty())
      o << misc::incendl << children_ << misc::decindent;
    return o;
  }
