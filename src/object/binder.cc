...
namespace object
{
  ...
  /* Handle the case of `self'.  If `self' is not found in the
     variable symbol table *and* this SimpleVar node is located under
     a ClassTy, then don't throw an error.  Variable `self' will have
  ...
  /* We can't bind methods definitions without types, so we don't
     ...
     type-checkimg, there's not need to visit method in two-pass (like
