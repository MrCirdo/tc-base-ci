...
namespace object
{
  ...
  // FIXME: Some code was deleted here (Initialize def).
    ...
    for (const type::Class* c : class_type->subclasses_get())
      if (ranges::any_of(decs->decs_get(), [c](const ast::TypeDec* tmp) {
  ...
  parse::Tweast* DesugarVisitor::method_call(const misc::symbol& class_name,
  ...
  void DesugarVisitor::operator()(const ast::DecsList& e)
  {
    ...
    for (const type::Class* c = cls->super_get(); c; c = c->super_get())
      {
        ...
        if (ranges::any_of(e.decs_get(), [c](const ast::TypeDec* t) {
        ...
        auto meth_it =
          ranges::find_if(c->meths_get(), [method](const type::Method* meth) {
  ...
  void DesugarVisitor::handle_class(const ast::TypeDecs& e,
                                    const type::Class* cls,
  ...
  /* All type-related code is emitted into the top-level decs-list, so
     ...
     of them.  */
  void DesugarVisitor::operator()(const ast::TypeDecs& e)
  {
    parse::Tweast functions;
  ...
  // FIXME: Some code was deleted here (Recurse).
    ...
    ast::Exp* attr_var =
      parse::parse(parse::Tweast()
                   << var << "." << variant_field_prefix << class_names_(owner)
                   << "."
  // FIXME: Some code was deleted here.
      );
    result_ = attr_var;
