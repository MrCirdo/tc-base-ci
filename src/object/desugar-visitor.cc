...
namespace object
{
                                  ...
                                  const type::Method* dispatch_method)
  {
    ...
    // declaration, formals of the mother method has been used
        ...
        for (auto c : overridings)
          {
            ...
            input
              << method_call(class_names_(nearest_c), method_name,
    ...
    for (const ast::Decs* d : e.decs_get())
      {
        ...
        if (decs)
        ...
        else
          {
            ...
            if (decslist)
              {
                misc::back_append_and_move(contents, decslist->decs_get());
                delete decslist;
              }
            else
              abort();
        ...
        for (const type::Class* c = cls->super_get(); c; c = c->super_get())
          {
            ...
            if (std::any_of(begin(e.decs_get()), end(e.decs_get()),
            ...
            auto meth_it
              = std::find_if(begin(c->meths_get()), end(c->meths_get()),
                             [m](const type::Method* meth) {
                               return meth->name_get() == m->name_get();
                             });

            if (meth_it == end(c->meths_get()))
              continue;

            // Since we're looping inside a chunk, we do not rebuild an
            // already built sub dispatch method.
            auto disp_it
              = std::find_if(begin(sub_dispatches), end(sub_dispatches),
                             [meth_it, c](pair_class_method pair) {
                               return (pair.first == c
                                       && pair.second == *meth_it);
                             });

            if (disp_it == end(sub_dispatches))
              sub_dispatches.emplace_back(c, *meth_it);
            else
              continue;
  ...
  // FIXME: Some code was deleted here (Initialize def).
            ...
            if (def->result_get())
              functions << " : " << recurse(def->result_get());
            functions << " = " << dispatch_switch(c, *meth_it, &e, true, m);
          }


        functions <<
          " function " << dispatch_fun_name(cls, m) <<
          " (self : " << class_variant_prefix << class_name;
        // Get the other arguments.
        const ast::MethodDec* def;
  // FIXME: Some code was deleted here (Initialize def).
        ...
        if (def->result_get() != nullptr)
          functions << " : " << recurse(def->result_get());
        functions << " = " << dispatch_switch(cls, m, &e);
      }
  }
  ...
  // FIXME: Some code was deleted here (Otherwise, desugar this FieldVar as an access to an attribute).
