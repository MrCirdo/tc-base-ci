...
namespace frame
{
  ...
  Frame::Frame(const temp::Label& label, const bool_list_type& formal_escapes)
    : label_(label)
  {
  ...
  temp::Label Frame::label_get() const { return label_; }

  const access_list_type& Frame::formals_get() const { return formals_; }

  int Frame::get_current_frame_size() const
  {
  ...
  const InFrame* Frame::frame_alloc()
  {
    int offset = -offset_;
    offset_ += word_size;
  ...
  const Access* Frame::alloc(bool escapes)
  {
    if (escapes)
      return frame_alloc();
    else
  ...
  const Access* Frame::local_alloc(bool escapes)
  {
  ...
  const Access* Frame::formal_alloc(bool escapes_p)
  {
  ...
  void Frame::formal_alloc(const bool_list_type& l)
  {
    for (const bool b : l)
      formal_alloc(b);
  }

  void Frame::arg_add(size_t n)
  {
