...
namespace frame
{
  ...
  tree::rExp InRegister::exp(const tree::rExp&) const
  {
    return new tree::Temp(temp_);
  }

  InRegister* InRegister::clone() const { return new InRegister(temp_get()); }

  std::ostream& InRegister::dump(std::ostream& ostr) const
  {
  ...
  tree::rExp InFrame::exp(const tree::rExp& fp) const
  {
    tree::rExp indir;
    if (offset_)
      indir =
        new tree::Binop(tree::Binop::Oper::add, fp, new tree::Const(offset_));
    else
      indir = fp;
    return new tree::Mem(indir);
  }

  InFrame* InFrame::clone() const { return new InFrame(offset_get()); }

  std::ostream& InFrame::dump(std::ostream& ostr) const
  {
