...
namespace target
{
               ...
               Target& target)
  {
    ...
    public:
      ...
      virtual void operator()(const tree::ProcFrag& frag) override
      {
        ...
        fragments_->push_back(proc_frag);
      }

      virtual void operator()(const tree::DataFrag& frag) override
      {
        // Build an assembly-data fragment.
        fragments_->push_back(new assem::DataFrag(frag));
      }
  ...
  {
    str << target.assembly_get().runtime(gc) << std::endl;
  }
