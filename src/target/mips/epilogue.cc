namespace target
{
  namespace mips
  {
    ...
    void
    Codegen::cpu_set(const target::mips::Cpu& cpu)
    {
      cpu_ = &cpu;
    }
    ...
    void
    Codegen::assembly_set(SpimAssembly& assembly)
    {
      assembly_ = &assembly;
    }
    ...
    Codegen::view_shift(assem::ProcFrag& frag)
    {
      ...
      temp::temp_list_type::const_iterator arg_iter;

      // Move registers argument to their dedicated location.
      for (formal_iter =  formals.begin(),  arg_iter  = args.begin();
           formal_iter != formals.end()  && arg_iter != args.end();
