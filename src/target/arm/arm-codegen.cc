...
#line 34 "target/arm/mem.brg"
  ...
  EMIT(ARM_ASSEMBLY.load_build(c.cast<Const>()->value_get(),
                                tree->asm_get()));
...
mono_burg_emit_26 (MBTREE_TYPE &tree, MBCGEN_TYPE *s)
{
  ...
  (void) src;
  {
#line 39 "target/arm/move.brg"

  EMIT(ARM_ASSEMBLY.move_build(src.cast<Const>()->value_get(),
                                dst->asm_get()));
