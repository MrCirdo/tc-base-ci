...
namespace target
{
  namespace arm
  {
                             ...
                             const temp::Temp& dst) const
    {
              ...
              && (right < 4096 || 16320 < right || right % 64 != 0)))
      {
        ...
        res.splice(res.end(), move_build(right, tp));
        res.splice(res.end(), binop_build(op, left, tp, dst));
        return res;
      }
      else
      {
        std::string opcode = binop_inst(op) + "'d, 's, #" + std::to_string(right);
        return { new assem::Oper(opcode, Ts(left), Ts(dst)) };
      }
    }
  ...
  // FIXME: Some code was deleted here.
    ...
    {
      return { new assem::Oper("b\t'j ", Ts(), Ts(), Ls(label)) };
    }
    ...
    {
      return { new assem::Oper("push\t{'s, 's}", Ts(t1, t2), Ts()) };
    }
    ...
    {
      return { new assem::Oper("pop\t{fp, pc}", Ts(), Ts()) };
    }
