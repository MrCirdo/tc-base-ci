...
namespace translate
{
  ...
  {
    tree::rExp address = index == 0
      ? var_exp->un_ex()
      : new tree::Binop(tree::Binop::Oper::add, var_exp->un_ex(),
  ...
  rExp nil_exp() { return new Ex(new tree::Const(0)); }
  ...
  rExp record_exp(std::vector<rExp> fields)
  {
  ...
  rExp call_exp(const std::string& label, rExp arg1, rExp arg2)
  {
  ...
  rExp array_exp(rExp array_size, rExp init_val)
  {
    ...
    temp::Temp array;
    return new Ex(
      new tree::Eseq(new tree::Move(new tree::Temp(array), content->un_ex()),
                     new tree::Temp(array)));
  }
  ...
  {
    temp::Temp rv("rv");
    rExp body = new Nx(new tree::Move(new tree::Temp(rv), value->un_ex()));
