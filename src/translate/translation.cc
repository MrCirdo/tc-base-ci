...
namespace translate
{
  ...
  record_exp(std::list<rExp> args)
  {
    ...
    int record_size = args.size() * frame::word_size;
    rExp malloc_call = call_exp("malloc", int_exp(record_size));
    init->push_back(new tree::Move(rec, malloc_call->un_ex()));
  ...
  rExp
  call_exp(const temp::Label& label, std::list<rExp> args)
  {
  // FIXME: Some code was deleted here.
  }
