...
namespace translate
{
  ...
  temp::Label Level::label_get() const { return frame_get().label_get(); }

  // In .cc because requires frame/frame.hh.
  int Level::frame_size_get() const { return frame_->get_current_frame_size(); }

  // Higher level interface to frame::local_alloc.
  const Access* Level::local_alloc(bool escapes)
  {
    const Access* access = new Access(*this, frame_->local_alloc(escapes));
    locals_.emplace_back(access);
    return access;
  }

  const Access* Level::formal_alloc(bool escapes)
  {
    const Access* access = new Access(*this, frame_->formal_alloc(escapes));
    formals_.emplace_back(access);
    return access;
  }

  tree::rExp Level::sl() const
  {
    // An access to the first formal, hopefully the static link.
    return (*frame_get().formals_get().begin())->exp();
  }

  tree::rExp Level::fp(const Level& use_level) const
  {
    if (&use_level == this)
    ...
    for (const Level* temp = use_level.parent_get();
         temp != this && temp != nullptr; temp = temp->parent_get())
      res = new tree::Mem(res);

    return res;
  }

  std::ostream& Level::dump(std::ostream& o) const
  {
    o << "translate::Level(" << misc::incendl << "formals = (" << formals_
      << "), "
      << "parent = ";
    if (parent_)
      o << parent_->label_get();
