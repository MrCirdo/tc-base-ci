/**
 ** \file src/translate/translator.cc
...
using namespace translate::tasks;
  ...
  tree::Fragments* Translator::fragments_get() { return fragments_; }

  rExp Translator::translate(const ast::Ast& node)
  {
    node.accept(*this);
    return exp_;
  }
  ...
  template <typename T>
  std::vector<rExp> Translator::translate(const std::vector<T*>& es)
  {
    std::vector<rExp> res;
    res.reserve(es.size());
    for (const T* i : es)
  ...
  void Translator::operator()(const ast::CastVar& e)
  {
  // FIXME: Some code was deleted here.
  }
  ...
  void Translator::operator()(const ast::NilExp&) { exp_ = nil_exp(); }
  ...
  void Translator::operator()(const ast::ForExp& e)
  {
  // FIXME: Some code was deleted here.
  }
  ...
  void Translator::operator()(const ast::LetExp& e)
  {
  ...
  void Translator::operator()(const ast::VarDecs& e)
  {
  ...
  void Translator::operator()(const ast::TypeDecs&) {}

  void Translator::operator()(const ast::FunctionDecs& e)
  {
  ...
  void Translator::operator()(const ast::FunctionDec&)
  {
    // We must not be here.
    unreachable();
  }
  ...
  void Translator::visit_function_dec_body(const ast::FunctionDec& e)
  {
    ...
    if (e.result_get())
      frag =
        function_dec(fun_label_[&e], e.name_get(), exp_, &level_->frame_get());
    else
      frag =
        procedure_dec(fun_label_[&e], e.name_get(), exp_, &level_->frame_get());
