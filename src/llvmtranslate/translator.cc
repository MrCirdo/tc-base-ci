...
namespace llvmtranslate
{
  ...
  namespace
  {
    ...
    {
      return llvm::TypeBuilder<llvm::types::i<32>, true>::get(ctx);
    }
                  ...
                  const std::string& name)
    {
      ...
      return tmp.CreateAlloca(ll_type, 0, name);
    }
  ...
  // FIXME: Some code was deleted here (Get the llvm type of the VarDec).
            args_types.push_back(llvm::PointerType::getUnqual(var_ltype));
          }
      }

    // Then, the actual arguments
    for (const auto field : function_type.formals_get())
      args_types.push_back(llvm_type(field.type_get()));
  ...
  {
    // An empty SeqExp is an empty expression, so 0
  // FIXME: Some code was deleted here.
  }
  ...
  Translator::operator()(const ast::WhileExp& e)
  {
    ...
    // Explicitily fall through from the current block
