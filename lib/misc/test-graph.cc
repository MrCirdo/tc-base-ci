...
consistency(graph_type& g)
{
  ...
  using neighb_iter_type = boost::graph_traits<graph_type>::adjacency_iterator;
  neighb_iter_type p;
  neighb_iter_type p_end;
  for (std::tie(p, p_end) = boost::adjacent_vertices(n, g);
       p != p_end; ++p)
  ...
  for (std::tie(i, i_end) = boost::vertices(g);
       i != i_end; ++i)
    if (removed.has(*i))
      return false;
